...
export function SettingsPage() {
  ...
  const update = <K extends keyof LauncherSettings>(key: K, value: LauncherSettings[K]) => setDraft((current) => ({ ...current, [key]: value }));
          ...
          <Panel className="p-5">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <label className="grid gap-2 text-sm">
                ...
                <input type="color" value={draft.accentColor ?? '#ff7a1a'} onChange={(event) => update('accentColor', event.target.value)} className="h-10 w-full rounded-md border border-white/10 bg-zinc-950/80 p-1" />
              </label>
              <label className="grid gap-2 text-sm">
              ...
              {draft.theme === 'custom' && (
                <label className="grid gap-2 text-sm md:col-span-2">
                  ...
                  <textarea value={draft.customThemeCss ?? ''} onChange={(event) => update('customThemeCss', event.target.value)} className="min-h-28 rounded-md border border-white/10 bg-zinc-950/80 p-3 font-mono text-xs outline-none" />
                </label>
              )}
            </div>
          </Panel>
          <Panel className="p-5">
            ...
            <div className="grid gap-4">
              <label className="grid gap-2 text-sm">
                ...
                <input className="range" type="range" min={1024} max={24576} step={512} value={draft.defaultRamMb ?? 4096} onChange={(event) => update('defaultRamMb', Number(event.target.value))} />
              </label>
              <label className="grid gap-2 text-sm">
                <span className="text-zinc-400">Java executable</span>
                <input value={draft.javaPath ?? ''} onChange={(event) => update('javaPath', event.target.value)} className="h-10 rounded-md border border-white/10 bg-zinc-950/80 px-3 outline-none" placeholder="Auto detect" />
              </label>
              <label className="grid gap-2 text-sm">
                <span className="text-zinc-400">Minecraft root</span>
                <input value={draft.minecraftRoot ?? ''} onChange={(event) => update('minecraftRoot', event.target.value)} className="h-10 rounded-md border border-white/10 bg-zinc-950/80 px-3 outline-none" placeholder="App data default" />
              </label>
              <label className="grid gap-2 text-sm">
                <span className="text-zinc-400">Parallel downloads</span>
                <input type="number" min={1} max={32} value={draft.maxParallelDownloads ?? 8} onChange={(event) => update('maxParallelDownloads', Number(event.target.value))} className="h-10 rounded-md border border-white/10 bg-zinc-950/80 px-3 outline-none" />
              </label>
            </div>
          </Panel>
          <Panel className="p-5">
            ...
            <div className="mt-4 grid gap-4">
              <input value={draft.microsoftClientId ?? ''} onChange={(event) => update('microsoftClientId', event.target.value)} className="h-10 rounded-md border border-white/10 bg-zinc-950/80 px-3 outline-none" placeholder="Microsoft public client ID" />
              <input value={draft.curseForgeApiKey ?? ''} onChange={(event) => update('curseForgeApiKey', event.target.value)} className="h-10 rounded-md border border-white/10 bg-zinc-950/80 px-3 outline-none" placeholder="CurseForge API key" />
      ...
      <aside className="grid min-h-0 content-start gap-5 overflow-y-auto pr-1">
        <Panel className="p-5">
          ...
          <div className="mt-4 grid gap-3">
            {javaRuntimes.map((runtime) => (
              <button key={runtime.path} onClick={() => update('javaPath', runtime.path)} className="rounded-lg border border-white/10 bg-white/[0.04] p-3 text-left transition hover:bg-white/[0.08]">
        ...
        <Panel className="p-5">
          ...
          <div className="grid gap-3">
            {plugins.map((plugin) => (
              <button key={plugin.id} onClick={() => window.dawn.plugins.toggle(plugin.id).then(setPlugins)} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.04] px-4 py-3 text-left">
        ...
        <Panel className="p-5">
          <Button className="w-full" icon={<FolderOpen size={16} />} onClick={() => draft.minecraftRoot && window.dawn.app.revealPath(draft.minecraftRoot)}>
            Reveal Minecraft Root
          </Button>
        </Panel>
      </aside>
    </div>
