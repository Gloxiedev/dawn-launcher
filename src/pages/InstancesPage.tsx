...
export function InstancesPage() {
  ...
  const query = librarySearchNonce > 0 ? librarySearch : localQuery;
  const selected = instances.find((instance) => instance.id === selectedInstanceId);
  const filtered = useMemo(
    () => instances.filter((instance) => instance.name.toLowerCase().includes(query.trim().toLowerCase())),
    [instances, query]
  );
    ...
    <PageShell aside={settingsAside} asideWidth={360}>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input
            className="h-10 rounded-md border border-white/10 bg-white/[0.06] px-3 text-sm outline-none"
            placeholder="Search instances"
            value={librarySearchNonce > 0 ? librarySearch : localQuery}
          ...
          <Button
            tone="primary"
            icon={<Plus size={16} />}
            onClick={() => void createInstance({ name: `Instance ${instances.length + 1}`, gameVersion: versions[0] || 'latest-release' })}
          >
            New
          </Button>
        </div>
      </div>
      <div className={layout === 'grid' ? 'grid grid-cols-1 gap-4 lg:grid-cols-2 2xl:grid-cols-3' : 'grid gap-3'}>
        {filtered.map((instance) => (
          <InstanceCard key={instance.id} instance={instance} />
        ))}
        {!filtered.length && <Panel className="p-8 text-center text-sm text-zinc-500">No instances match your search.</Panel>}
      </div>
    </PageShell>
  );
}
