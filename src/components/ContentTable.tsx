...
export function ContentTable({ instanceId, kind, query = '' }: ContentTableProps) {
  ...
  useEffect(() => {
    const unsubscribe = window.dawn.events.onContentChanged((event) => {
      if (event.instanceId === instanceId && event.kind === kind) {
        void refresh();
      }
    });
    return unsubscribe;
  }, [instanceId, kind, refresh]);

  return (
    <Panel className="flex min-h-[280px] flex-col overflow-hidden lg:h-full">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        {filtered.map((file) => (
          <div key={file.path} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 border-b border-white/[0.06] px-4 py-3">
            <div className="flex min-w-0 items-center gap-3">
              <FileArchive size={18} className="shrink-0 text-orange-200" />
        ...
        {!filtered.length && (
          <div className="p-8 text-center text-sm text-zinc-500">{files.length ? 'No files match the search.' : 'No files installed.'}</div>
        )}
      </div>
    </Panel>
  );
